(function(){
  ...
  if(!digs.s) return;
  function cel(){ var c = document.createElement('span'); c.className = 'cel'; var t = document.createElement('span'); t.className = 'tira';
    var h = '<i>9</i>'; for(var k=0;k<10;k++) h += '<i>'+k+'</i>'; t.innerHTML = h; c.appendChild(t); return c; }
  Object.keys(digs).forEach(function(u){ digs[u].appendChild(cel()); digs[u].appendChild(cel()); });
  var semRolar = matchMedia('(prefers-reduced-motion:reduce)').matches;
  function partes(){ var s = Math.max(0, Math.floor((FIM - Date.now())/1000));
    return {d:Math.floor(s/86400), h:Math.floor(s%86400/3600), m:Math.floor(s%3600/60), s:s%60, total:s}; }
  function poe(t, d, anim){
    var ant = +t.getAttribute('data-d'); if(ant === d) return; t.setAttribute('data-d', d);
    if(anim && ant === 0 && d === 9){ t.style.setProperty('--n', 0);
      setTimeout(function(){ t.classList.add('salta'); t.style.setProperty('--n', 10); void t.offsetWidth; t.classList.remove('salta'); }, 600); return; }
    if(!anim) t.classList.add('salta');
    t.style.setProperty('--n', d + 1);
    if(!anim){ void t.offsetWidth; t.classList.remove('salta'); }
  }
  var srEl = sec.querySelector('[data-relogio-sr]'), ultSr = '';
  function desenha(anim){ var p = partes();
    ['d','h','m','s'].forEach(function(u){ var val = Math.min(p[u], 99), t = digs[u].querySelectorAll('.tira');
      poe(t[0], Math.floor(val/10), anim && !semRolar); poe(t[1], val%10, anim && !semRolar); });
    var k = p.d + '-' + p.h + '-' + p.m;
    if(srEl && k !== ultSr){ ultSr = k; srEl.textContent = 'Faltam ' + p.d + ' dias, ' + p.h + ' horas e ' + p.m + ' minutos.'; }
    return p; }
  desenha(false);
  var tId = null;
  function tick(){ var p = desenha(true); if(p.total > 0) tId = setTimeout(tick, 1000 - (Date.now() % 1000) + 8); }
  tId = setTimeout(tick, 1000 - (Date.now() % 1000) + 8);
  document.addEventListener('visibilitychange', function(){
    if(document.hidden){ clearTimeout(tId); tId = null; } else if(!tId){ desenha(false); tId = setTimeout(tick, 1000 - (Date.now() % 1000) + 8); } });
  window.addEventListener('pageshow', function(){ desenha(false); });
})();
;
(function(){
